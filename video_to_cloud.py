...
def render_cloud_from_video(video_path, output_image_path, exclude_obvious):
    with tempfile.TemporaryDirectory() as dir:
        filename = os/path.split(video_path)[-1]
        temp_audio_path = os.path.join(dir.path, filename, '.ogg')
        convert_video_to_audio_moviepy(video_path, temp_audio_path)
        speech_to_cloud.render_cloud_from_audio(temp_audio_path, ooutput_image_path, exclude_obvious)
...
if __name__ == '__main__':
    ...
    render_cloud_from_video(args.video, args.output, args.remove_obvious)